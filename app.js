// ── STORAGE ───────────────────────────────────────────────────────────────────
const STORAGE_KEY = 'casting_bible_v2';
...
function filterTalents() {
  return db.talents.filter(p => {
    ...
    if (searchQ) {
      ...
      const match = [p.nom,p.prenom,p.sport,p.agence,p.pays,p.ville,p.insta,p.notes,
    ...
    if (fAgence && p.agence !== fAgence) return false;
    return true;
  });
}
...
function renderTalents(main) {
  ...
  const sports  = [...new Set(scope.map(p=>p.sport).filter(Boolean))].sort();
  const pays    = [...new Set(scope.map(p=>p.pays).filter(Boolean))].sort();
  const agences = [...new Set(scope.map(p=>p.agence).filter(a=>a&&a!=='—'&&a!==''))].sort();
  ...
  if (!list.length) {
    ...
    list.forEach(p => {
      ...
      const bgStyle = p.photo ? '' : `background:${getCatBg(p.cats||[])};`;

      html += `<div class="card" onclick="openDetail('talent',${p.id})">`;

      if (p.photo) {
        html += `<img class="card-img" src="${esc(p.photo)}" alt="${esc(p.nom)}"
      ...
      html += `<div class="card-body">
        ...
        <div class="card-meta">
          ...
          ${p.agence && p.agence !== '—' && p.agence ? `<span class="tag agence">${esc(p.agence)}</span>` : ''}
...
function renderMarques(main) {
  ...
  db.marques.forEach((m, i) => {
    html += `<span class="marque-tag" title="Cliquer pour éditer" onclick="editMarque(${i})">${esc(m)}</span>`;
  });
...
function resetAll() {
  if (!confirm('Effacer TOUTES les données et repartir des données initiales ?')) return;
  ...
  if (type === 'talent') {
    body = `<div class="detail-layout">
      <div class="detail-img">
        ${p.photo
          ? `<img src="${esc(p.photo)}" alt="${esc(p.nom)}"
              onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
      ...
      <div class="detail-info">
        ...
        ${p.agence && p.agence !== '—' && p.agence ? `<div class="detail-row"><span class="detail-lbl">AGENCE</span><span class="detail-val">${esc(p.agence)}</span></div>` : ''}
...
function openForm(type, pid) {
  ...
  if (type === 'talent') {
    ...
    const val = isEdit ? db.marques[pid] : '';
...
function renderTalentForm(v) {
  const ags   = agenceNames().map(a => `<option value="${esc(a)}"${(v.agence||'')===a?' selected':''}>${esc(a)}</option>`).join('');
  ...
  document.getElementById('form-body').innerHTML = `
    ...
    <div class="field"><label>PAYS</label>
      <select id="f-pays" onchange="syncVilles()">${paysl}</select>
    </div>
    <div class="field"><label>VILLE</label><select id="f-ville">${villesl}</select></div>
    ...
    <div class="field full">
      <label>PHOTO</label>
      <input type="hidden" id="f-photo" value="${esc(v.photo||'')}">
      <div class="photo-upload-row">
        <label class="btn-upload" for="f-photo-file">📷 CHOISIR UNE PHOTO</label>
        <input type="file" id="f-photo-file" accept="image/*" style="display:none" onchange="handlePhotoUpload(this)">
        <span id="photo-filename" class="photo-filename"></span>
        ${v.photo ? `<button type="button" class="btn-clear-photo" onclick="clearPhoto()">✕ Retirer</button>` : ''}
      </div>
      <img id="photo-preview" class="photo-preview-img ${v.photo?'show':''}" src="${esc(v.photo||'')}" alt="">
    </div>
...
// Compresse l'image côté navigateur avant de la stocker en base64 (localStorage friendly)
function handlePhotoUpload(input) {
  ...
  reader.onload = function(e) {
    const img = new Image();
    img.onload = function() {
      // Redimensionne à max 700px de large, garde le ratio, compresse en JPEG ~0.75
      const maxW = 700;
      ...
      const hidden = document.getElementById('f-photo');
...
function submitForm() {
  ...
  if (type === 'talent') {
    ...
    if (isEdit) db.talents = db.talents.map(p => p.id === pid ? { ...obj, id: p.id } : p);
    ...
    if (!g('f-nom')) { alert('Le nom est requis.'); return; }
    const obj = { nom:g('f-nom'), pays:g('fa-pays')||'', ville:g('fa-ville')||'', mail:g('f-mail'), notes:g('f-notes') };
    if (isEdit) db.agences = db.agences.map(a => a.id === pid ? { ...obj, id: a.id } : a);
    else        db.agences.push({ ...obj, id: uid() });

  } else if (type === 'marque') {
    if (!g('f-nom')) { alert('Le nom est requis.'); return; }
    if (isEdit) db.marques[pid] = g('f-nom');
    else        db.marques.push(g('f-nom'));
  }
